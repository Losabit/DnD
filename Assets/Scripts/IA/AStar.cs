...
public static class AStar
{
    ...
    public static List<Vector2> execute(int[][] map, Vector3 posStart, Vector3 posEnd)
    {
        return execute(map, new Vector2(posStart.x, posStart.z), new Vector2(posEnd.x, posEnd.z));
    }

    public static List<Vector2> execute(int[][] map, Vector2 posStart, Vector2 posEnd)
    {
        ...
        do
        {
            ...
            if (nodes[indice].DistanceEnd.Equals(0))
            {
                ...
                List<Node> path = GetBestPath(nodes, posStart, posEnd);
        ...
        List<Node> bestPath = GetBestPath(nodes, posStart, posEnd);
    ...
    private static List<Node> GetBestPath(List<Node> nodes, Vector2 posStart, Vector2 posEnd)
    {
    ...
    private static List<Node> InitializeDijkstraList(int[][] map, Vector2 posStart, Vector2 posEnd)
    {
        ...
        for (int z = 0; z < map.Length; z++)
        {
            ...
            {
                if (!map[z][x].Equals(wallValue))
                {
                    Node node = new Node();
                    node.position = new Vector2(x, z);
    ...
    private static int NumberOfNodes(int[][] map)
    {
        int result = 0;
        for (int z = 0; z < map.Length; z++)
        {
